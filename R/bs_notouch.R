bs_notouch <-
function (payoff=1, spot, strike, barrier, v, r, rf, t, c_p = "call", pay_time=1, in_out=0, rate_type="nominal") {
  #Pays domestic cash amount if barrier is NOT hit before expiration. 
  ...
  cp = (c_p == "call" | c_p == "c" | c_p == "CALL" | c_p == "C" | c_p == "Call") * 2 - 1
    if(in_out==1){price_notouch = 0
      return(price_notouch)}
  if(B<spot){n=1};if(B>spot){n=-1}#Lower or upper barrier
  r_c = r
  rf_c= rf
