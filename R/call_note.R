#' Call note
#'
#' Valoracion de call note
#' @param ini_date initial date
#' @return valuation
#' @export

call_note <-function(ini_date, mat_date, fix_index_date, fix_spot_date, ini_date_avg, fin_date_avg,
                     fx_spot, index_spot, index_ini, a, b, nom_pay, nom_prot, bond_cc_curve, dom_cc_curve, for_cc_curve,
                     vol_matrix, base, coupon=0, credit_spread=0, fwd_points, haz_curve=NULL,
                     ...
                     vol_matrix_quanto=NULL, quanto_cc_curve=NULL, rho=0, prev_vol=NULL, adj_strike=0, payoff_type=2, index_series=NULL, float_strike=FALSE, geometric=FALSE, settDays, vol_factor=1, M=1e4, delta_vol_max=NA, drift_cero=FALSE, rec=0.4){
  ...
  if(!is.null(prev_vol) & !is.na(delta_vol_max)){
  ...
  if(type=="v"){
    call_pr=(nom_call*b/index_ini)*bs_quanto(spot=index_spot, strike=strike+adj_strike, quanto=quanto, v=vol, vq=vol_quanto, rho=rho, r=r_call, rf=rf, rq=rq, tn=tn, c_p = "call", rate_type = rate_type)
  }
  if(type=="a"){
    call_pr=(nom_call*b/index_ini)*bs_asian(ini_date=ini_date,spot=index_spot, strike=strike+adj_strike, quanto=quanto, v=vol, vq=vol_quanto, rho=rho, r=r_call, rf=rf, rq=rq, mat_date=mat_date, ini_date_avg=ini_date_avg, fin_date_avg=fin_date_avg, c_p = "call", float_strike=float_strike, geometric=geometric, rate_type = rate_type,  base=base, M=M, index_series=index_series, settDays=settDays)
  }
